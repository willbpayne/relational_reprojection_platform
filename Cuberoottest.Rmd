...
dataFile <- geojson_sf("app/data/admin1-us_v2.geojson")

# this converts it to a dataframe of just points
TXdataframe = data.frame(st_coordinates(st_cast(dataFile$geometry,"MULTIPOINT")))
...
Polygon_List<-split(TXdataframe, TXdataframe$L1_Chr)
...
for (i in 1:length(Polygon_List))
  p = p + geom_polygon(aes(x=cuberootdistancex, y=cuberootdistancey), Polygon_List[[i]], rule = 'winding', fill = 'NA', colour = "black", size = .2)
p
