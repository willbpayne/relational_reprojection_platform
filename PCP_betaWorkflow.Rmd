...
```{r} 
#temp ***
df2$valTrans <- (df2$val / 500) + 1 # manual one I did for India data to see what we're looking for
...
df2 <- select(df2,-starts_with("circdistance"))
class(df2$circcoords)
...
for (row in 1:nrow(df2)){
  df2[row,"distancelogcircx"] <- (useful::pol2cart(df2$distancelog[row], df2$ctrPtMathbearing[row], degrees = TRUE))[1]
  df2[row,"distancelogcircy"] <- (useful::pol2cart(df2$distancelog[row], df2$ctrPtMathbearing[row], degrees = TRUE))[2]
}
...
for (row in 1:nrow(df2)){ 
  df2[row,"sqrtcircX"] <- (useful::pol2cart(df2$distancesqrt[row], df2$ctrPtMathbearing[row], degrees = TRUE))[1]
  df2[row,"sqrtcircY"] <- (useful::pol2cart(df2$distancesqrt[row], df2$ctrPtMathbearing[row], degrees = TRUE))[2]
}
rm(row)
