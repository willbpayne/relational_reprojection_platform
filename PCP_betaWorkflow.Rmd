...
latNames <- list("lat","Lat","LAT", "latitude", "Latitude", "LATITUDE", "y","Y") # add as they come up
lonNames <- list("lon","Lon","LON","long","Long","LONG","longitude", "Longitude", "LONGITUDE", "x","X")
...
```{r} 
#temp ***
df2$valTrans <- (df2$val / 800)^1.5 # manual one I did for NH data to see what we're looking for
...
plot07A <- ggplot(df2, aes(
                df2$logcoords[,1], 
                df2$logcoords[,2]),
                 color = df2$distance) +  
  geom_circle(aes(x0 = x0, y0 = y0, r = log(r)), 
                      colour = "#eeeeee", data = circles, 
                      show.legend = NA, inherit.aes = FALSE) +
  geom_point(stroke = 1, size = df2$valTrans, color = df2$distance) + 
  geom_point(data = (as.data.frame(ctrPt)), aes(0, 0), color = "orange") +
  geom_text(data = df2,
              aes(
                  df2$logcoords[,1], 
...
plot07B <- ggplot(df2, aes(
                df2$logcoords[,1], 
                df2$logcoords[,2], 
                color = df2$val),
                arrange(df2, desc(val))) + 
  geom_circle(aes(x0 = x0, y0 = y0, r = log(r)), 
...
plot07B
 ...
 geom_text(data = routesSub,
                    aes(lagrangedistcircX,
                        ...
                        color = routesSub$DTWconnectivity),
                    size = 1.5,
                    check_overlap = TRUE) +
...
#                    geom = "text", 
#                    x = ctrPt[1], y = ctrPt[2], 
#                    label = routes$airport[routes$isDTW == TRUE],
#                    color = "orange",
#                    size = 2,
#                    vjust = 1.25,
#                    hjust = -0.25) +
