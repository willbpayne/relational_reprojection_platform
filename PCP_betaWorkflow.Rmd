...
convertToPolar <- function(dataFrame, centerPoint,latCol,lonCol){
   # valNamelonglat <- c(lonCol, latCol)
    # dataFrame$ctrPtGeobearing <- geosphere::bearing(centerPoint, valNamelonglat, a=6378137, f=1/298.257223563)
  #rm(valNamelat,valNamelong,valNamelonglat,row)
    df2 = mutate(dataFrame, emptyCol=NA)
  return(df2)
}
convertToPolar(df2, ctrPt, df2$lat, df2$lon)
convertToPolar(df2,ctrPt)
