...
for (col in 1:ncol(df)) {
  ...
  { df2$ctrBin <- as.logical(df[[col]])
    print("Found a ctrBin column!")
    print(names(df)[[col]])}
  
  if (typeof(df[[col]]) == "character" || # right now catches both name and name_long
  is.factor(df[[col]]) == T) # valName
  { df2$valName <- as.character(df[[col]])
    print("Found a valName column!")
    print(names(df)[[col]])}
