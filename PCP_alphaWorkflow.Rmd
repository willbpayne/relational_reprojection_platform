...
geodist <- c(0, 500, 1000, 3500) # let's say under 500 km is close, over 1000km is far
chartdist <- c(0, 400, 800, 1200) # this just gets us equal intervals on the graph for the different segments of the lines

dat <- data.frame(cbind(geodist, chartdist))
 
ggplot(dat, aes(x=chartdist, y=geodist)) + 
  geom_point(size=5, col='blue')

library(polynom)
lagrangepoly <- poly.calc(geodist, chartdist)

predict(lagrangepoly, 2000)
