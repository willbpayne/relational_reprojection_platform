...
geogdist <- c(0, 500, 2000, maxdist) # let's say under 500 km is close, over 1000km is far
chartdist <- c(0, 400, 800, 1200) # this just gets us equal intervals on the graph for the different segments of the lines

dat <- data.frame(cbind(geogdist, chartdist))
ggplot(dat, aes(x=chartdist, y=geogdist)) + geom_point(size=5, col='blue') # chart points
...
for (row in 1:nrow(routes)){
  routes[row,"lagrangedistcircX"] <- (pol2cart(routes$lagrangedistcirc[row], routes$crtPtMathbearing[row], degrees = TRUE))[1]
  routes[row,"lagrangedistcircY"] <- (pol2cart(routes$lagrangedistcirc[row], routes$crtPtMathbearing[row], degrees = TRUE))[2]
}

pol2cart(routes$distance[2], routes$crtPtMathbearing[2], degrees = TRUE)

pol2cart(routes$lagrangedistcirc[2], routes$crtPtMathbearing[2], degrees = TRUE)

pol2cart(routes$lagrangedistcirc[2], routes$crtPtMathbearing[2], degrees = TRUE)[1]

pol2cart(routes$lagrangedistcirc, routes$crtPtMathbearing, degrees = TRUE)[1]


pol2cart(2684, 202.5643, degrees = TRUE)[1]

plot6 <- ggplot(routes, aes(lagrangedistcircX, lagrangedistcircY, color = routes$distance)) + geom_point(stroke = 1, size = routes$DTWconnectivity) + geom_point(data = (as.data.frame(ctrPt)), aes(0, 0), color = "orange") + coord_fixed() + labs(color = "Great circle distance in km") + xlim(-1200,1200) + ylim(-1200,1200)
