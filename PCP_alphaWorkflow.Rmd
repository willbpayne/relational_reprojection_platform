...
# assign variable to max dist
  ...
  maxdistY <- max(routes$distanceY) # longest NS dist
  ...
  if (maxdistX > maxdistY)
    {cat("Longest axial distance from center is", maxdist, "km east-west. ")
    if (maxXcoord < ctrX)
      {cat("Farthest point is", maxdistX, "km west of center point."); maxdistBearing <- 270}
    if (maxXcoord > ctrX)
      {cat("Farthest point is", maxdistX, "km east of center point."); maxdistBearing <- 90}}
  if (maxdistY > maxdistX)
    {cat("Longest axial distance from the center point is", maxdist, "km north-south. ")
    if (maxYcoord < ctrY)
      {cat("Farthest point is", maxdistY, "km south of center point."); maxdistBearing <- 180}
    if (maxYcoord > ctrY)
      {cat("Farthest point is", maxdistY, "km north of center point."); maxdistBearing <- 0}}
