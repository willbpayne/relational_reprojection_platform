...
# convert to polar coordinates (WE NEED TO DO THIS EARLY ON AND THEN STORE THE BEARING ALONG WITH GREAT CIRCLE DISTANCE, THEN WE TRANSFORM DISTANCE USING GREAT CIRCLE DISTANCE, AND FINALLY REPLOT USING pol2cart and the new distance)

cart2pol(-2347.424429, -975.4223676, degrees = TRUE) # convert from cartesian coordinates to polar

# p1 <- c(-83.35340, 42.21240) # DTW (long then lat) NOT NEEDED ANYMORE
p2 <- c(-112.0120, 33.43430) # PHX (long then lat)

routes$longlat <- list(c(routes$long, routes$lat)) # WHY DOESN'T THIS WORK?

routes$ctrPtbearing <- bearing(ctrPt, c(routes$long, routes$lat), a=6378137, f=1/298.257223563) # great circle distance should be 2684


bearing(ctrPt, p2, a=6378137, f=1/298.257223563) # great circle distance should be 2684

# if geobearing is negative, mathbearing is 90 plus bearing
...
ggplot(dat, aes(x=chartdist, y=geogdist)) + 
  geom_point(size=5, col='blue')

library(polynom)
lagrangepoly <- poly.calc(geogdist, chartdist)

lagrangepoly
predict(lagrangepoly, 200)

# routes$lagrangedistX <- (3 * routes$distanceX)

routes$lagrangedistX <- predict(lagrangepoly, routes$distanceX)
routes$lagrangedistY <- predict(lagrangepoly, routes$distanceY)
...
plot7a <- ggplot(routes, aes(x = aesX, y = aesY, 
                          z = routes$flightsDTW)) +
...
plot1a <- ggplot(routes, 
  ...
  geom_text(label=routes$airport) + 
