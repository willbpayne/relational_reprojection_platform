...
for (row in 1:nrow(routes)){ # GENERALIZE THIS FUNCTION
...
routes$lagrangedistcircstep <- a(routes$distance) # set new lagrange great circle distances using new stepwise function
...
for (row in 1:nrow(routes)){ # calculate new coordinates using lagrange distances
  routes[row,"lagrangedistcircX"] <- (pol2cart(routes$lagrangedistcircstep[row], routes$ctrPtMathbearing[row], degrees = TRUE))[1] # returns x coordinate only
  routes[row,"lagrangedistcircY"] <- (pol2cart(routes$lagrangedistcircstep[row], routes$ctrPtMathbearing[row], degrees = TRUE))[2] # returns y coordinate only
}
rm(row)

# NEED TO PROJECT CIRCLES TOO!

plot10A <- ggplot(routes, aes( # plot with maxdist/10 circles
                lagrangedistcircX, 
                lagrangedistcircY, 
                color = routes$distance)) +  
          geom_circle(aes(x0 = x0, y0 = y0, r = r), 
                      colour = "orange", data = lagrange_predictstep(circles), 
                      show.legend = NA, inherit.aes = FALSE) + 
          geom_point(stroke = 1, size = routes$DTWconnectivity) + 
          geom_point(data = (as.data.frame(ctrPt)), aes(0, 0), color = "orange") + 
          coord_fixed() + 
...
routesSub <- routes[!(routes$isDTW == TRUE), ]
