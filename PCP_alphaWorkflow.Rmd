...
plot3 <- ggplot(routes, aes(routes$distanceXlog * longDiffSign, routes$distanceYlog * latDiffSign, color = routes$distance)) + geom_point(stroke = 1, size = routes$DTWconnectivity) + geom_point(data = (as.data.frame(ctrPt)), aes(0, 0), color = "orange") + coord_fixed() + labs(color = "Great circle distance in km")
