...
```{r}
# first plot with ctrPt and data visualized
ggplot(routes, 
       aes(routes$long, 
           routes$lat, 
...
plot12 <- ggplot(data = f_df, aes(x = f_df$f_x, y = flts$f_y)) +
  geom_tile(aes(fill=flts$f_z)) +
  ggtitle("test plot") +
  scale_fill_continuous(name = "flights", low = "white", high = "blue")

plot12

#####
# dataframe as manually flattened interpolation
# doesn't currently work
flts_df <- as.data.frame(flts)
flts_df["Zgrid"] <- rowSums(flts_df[c("z.1","z.2","z.3","z.4","z.5",
                  ...
                  na.rm = TRUE)
plot14 <- ggplot(data = flts_df, aes(x=flts_df$x,
                                     y=flts_df$y,
                                     z=flts_df$Zgrid)) +
                  geom_tile(aes(fill = flts_df$Zgrid),
                            position = "identity") +
...
plot15 <- ggplot(data = flts_grid_df, aes(x = ldcX, y = ldcY, z = flights)) +
...
flts_df <- melt(flts_df, id=flts_df$x)
plot13 <- ggplot(data = flts, aes(x = flts$x,
               y = flts$y)) +
  geom_tile(aes(fill=flts$z)) +
  ggtitle("test plot") +
  scale_fill_continuous(name = "flights", low = "white", high = "blue")
