...
server <- function(input, output) {
  ...
  output$selectedData <- renderTable(
   ...
   maxdistfinder <- function(selected_dataframe) { # First non-reactive function! We copied a bunch o code for this
   ...
   ctrPtFinder <- function(selected_dataframe) { # Second non-reactive function! still super duplicated. Ugh.
     ...
     df2$distance <- geodist(ctrPt[1], ctrPt[2], df2$lat, df2$lon, units = "km")
     maxdist <- max(df2$distance) # max great circle distance
     valName <- colnames(df2$val)
     
     my_list <- list(ctrPt[1], ctrPt[2], ctrPtName)
     return(my_list)
   }  
 
   valfinder <- function(selected_dataframe) { # First non-reactive function! We copied a bunch o code for this
     df <- selected_dataframe
     df_ext <- ".csv"
